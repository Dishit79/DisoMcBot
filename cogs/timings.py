...
with open("cogs/data/timings_check.yml", 'r', encoding="utf8") as stream:
    try:
...
class Timings(commands.Cog):
                ...
                if "servers" in TIMINGS_CHECK:
                    for server in TIMINGS_CHECK["servers"]:
                        if server["name"] in version:
                            embed_var.add_field(**self.create_field(server))
                            break
                                ...
                                for plugin_name in TIMINGS_CHECK["plugins"][server_name]:
                                    if plugin == plugin_name:
                                        ...
                                        if isinstance(stored_plugin, dict):
                                            ...
                                            embed_var.add_field(**self.create_field(stored_plugin))
                                        else:
                                            self.eval_field(embed_var, stored_plugin, plugin_name, plugins,
                ...
                if "config" in TIMINGS_CHECK:
                    for config_name in TIMINGS_CHECK["config"]:
                        ...
                        for option_name in config:
                            ...
                            self.eval_field(embed_var, option, option_name, plugins, server_properties, bukkit,
    ...
    def eval_field(self, embed_var, option, option_name, plugins, server_properties, bukkit, spigot, paper, tuinity, purpur):
            ...
            for option_data in option:
                ...
                if add_to_field:
                    ...
                    option_data["value"] = eval('f"""' + option_data["value"].replace("\n", "\\|n\\") + '"""').replace(
                        "\\|n\\", "\n")
                    embed_var.add_field(**self.create_field({**{"name": option_name}, **option_data}))
                    break

        except KeyError as key:
            print("Missing: " + str(key))

    def create_field(self, option):
    ...
    @commands.command(brief="Checks your timings")
    @commands.has_permissions(manage_messages=True)
    async def check(self,ctx):
        if not "https://timings.aikar.co/" in ctx.message.content:
            return await ctx.reply("Please provide timings")
        await self.analyze_timings(ctx.message)
